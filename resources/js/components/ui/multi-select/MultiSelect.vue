<script setup lang="ts">
  import { ComboboxAnchor, ComboboxGroup, ComboboxItem, ComboboxItemIndicator } from "@/registry/default/ui/combobox";
  import { LucideCheck, LucideX } from "lucide-vue-next";
  import { ComboboxInput, ComboboxRoot, TagsInputInput, TagsInputItem, TagsInputItemDelete, TagsInputItemText, TagsInputRoot, useFilter } from "reka-ui";
  ...
  const filteredOptions = computed(() => options.filter((option) => contains(option.value, query.value) && !modelValue.value.some((item) => item.value === option.value)));
  ...
  <ComboboxRoot v-model="modelValue" multiple ignore-filter>
    ...
    <ComboboxList class="w-(--reka-combobox-trigger-width)">
      ...
      <ComboboxGroup v-if="filteredOptions.length">
        <ComboboxItem v-for="option in filteredOptions" :key="option.value" :value="option" :disabled="option.disabled">
          {{ option.label }}
