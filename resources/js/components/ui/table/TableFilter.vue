<script setup lang="ts">
  ...
  const columnHeader = computed(() => (typeof props.column.columnDef.header === "string" ? props.column.columnDef.header : ""));
    ...
    <template v-if="filterVariant === 'range'">
      ...
      <div class="flex">
        <Input
          ...
          @update:model-value="(value: string | number) => column.setFilterValue((old: [number, number]) => [value ? Number(value) : undefined, old?.[1]])"
          placeholder="Min"
          type="number"
        ...
        <Input
          ...
          @update:model-value="(value: string | number) => column.setFilterValue((old: [number, number]) => [old?.[0], value ? Number(value) : undefined])"
          placeholder="Max"
          type="number"
