...
<template>
  ...
  >
    <DateRangePickerCalendar v-slot="{ weekDays, grid }" class="max-h-[inherit] overflow-auto p-2">
      ...
      <DateRangePickerGrid v-for="month in grid" :key="month.value.toString()" class="w-full border-collapse space-y-1 select-none">
        <DateRangePickerGridHead>
        ...
        <DateRangePickerGridBody class="[&_td]:px-0 [&_td]:py-px">
          <DateRangePickerGridRow v-for="(weekDates, index) in month.rows" :key="`weekDate-${index}`" class="flex w-full">
